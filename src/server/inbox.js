...
import { imapOptions } from './config.js';
...
export async function searchMessages(cfg, criteria = {}) {
  const cap = Math.min(Math.max(Number(criteria.limit) || 20, 1), 50);
  return withClient(cfg, async (client) => {
...
export async function readMessage(cfg, { uid, folder } = {}) {
  ...
  return withClient(cfg, async (client) => {
    ...
    try {
      ...
      if (!msg || !msg.source) throw new Error('No he trobat cap missatge amb uid ' + id + ' a "' + path + '".');
